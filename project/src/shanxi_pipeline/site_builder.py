...
#
# 判据只有一条：它在原书语境里指的不是食材。现存两类——
#   ① 季节/条件词：原书给同一味料按季节列了三个用量，季节词被当成了名字；
#   ② 概数词残尾：「约一两」的「约」黏在名字后，而名字只有一个字，尾巴剥不掉。
#
# 曾经的第三类「名称粘连」（原书横排流水地把「甲少许／适量 乙 用量」排在一行，
# 配对切分把「甲+量词+乙」整段当成一个名字）已在 2026-07-29 从根因上修掉：
# recipe_segmenter 把模糊用量（少许/适量/微量/少量/若干/酌量）也当作断点，
# 并补齐了表外量词「厘段头」，「盐少许粉面 四两」现在正常断成「盐 少许」+「粉面 四两」。
# 原先列在这里的 14 条粘连写法（葱一段姜／味精五厘绍酒／糖五饯绍酒／盐少许粉面／
# 味精适量酱油／酱汤适量菜油／清油适量糖／糖色少许盐／绍酒少许鸡蛋／酱油少许味精／
# 菜油少许盐／菜油适量盐／菜油适量绍酒／蜂蜜少许菜油／面粉少许酱油）全库已不再产出，
# 一并删除——排除名单只治症状，根因修好后留着它们反而会掩盖回归。
#
...
INGREDIENT_NOT_FOOD = frozenset(
    {
        ...
        #    名字只剩一个字，_TAIL_QUALIFIER 不敢剥（剥完只剩「碱」以外的单字残渣更糟）。
        "碱约",
    }
)
